...
- highlight_color (string; default 'red'): Color of the data points
    ...
    mh = _ManhattanPlot(
        ...
        additional_highlight_row=additional_highlight_row,
        additional_highlight_val=additional_highlight_val,
    )
...
class _ManhattanPlot():
    ...
    def __init__(
            ...
            logp=True,
            # in_pathway=None,
            additional_highlight_row=None,
            additional_highlight_val=None,
    ):
        """
        Keyword arguments:
        ...
        #     if in_pathway not in x.columns.values:
        #         # Warn if you don't have an in_pathway column
        #         raise KeyError(
        #             "in_pathway argument specified as %s but column not "
        #             "found in 'x' data.frame" % in_pathway
        #         )
        #     else:
        #         # If the input DataFrame has an in_pathway column, add it to the new
        #         # DataFrame
        #         self.data[in_pathway] = x[in_pathway]
        ...
        self.logp = logp
        # self.inPathwayName = in_pathway
        self.additionHighlightValName = additional_highlight_val
        self.additionHighlightRowName = additional_highlight_row
    ...
    - A figure formatted for plotly.graph_objects.
        ...
        if highlight:
            if not isinstance(highlight, bool):
                ...
                ### Mostly we might want to edit here
                if not tmp.empty:
                    data_to_plot.append(
                        go.Scattergl(
        ...
        if self.nChr == 1:
            ...
            if col is None:
                ...
                for i in range(self.nChr):
                    colors.append('black' if np.mod(i, 2) else 'grey')
                col = colors
            
            for i in data[self.index].unique():

                tmp = data[data[self.index] == i]
                if isinstance(self.additionHighlightValName, str):
                    self.additionHighlightValName = [self.additionHighlightValName]


                chromo = tmp[self.chrName].unique()  # Get chromosome name
                if self.additionHighlightRowName is not None and self.additionHighlightValName is not None:
                    additional_highlight = tmp[self.additionHighlightRowName].isin(self.additionHighlightValName)
                    additional_highlight_data = tmp[additional_highlight]
                    not_additional_highlight_data = tmp[~additional_highlight]


                    hover_text_additional_highlight = _get_hover_text(
                        additional_highlight_data,
                        snpname=self.snpName,
                        genename=self.geneName,
                        annotationname=self.annotationName
                    )

                    hover_text_not_additional_highlight = _get_hover_text(
                        not_additional_highlight_data,
                        snpname=self.snpName,
                        genename=self.geneName,
                        annotationname=self.annotationName
                    )
                    ...
                    data_to_plot.append(
                        go.Scattergl(
                            x=not_additional_highlight_data[self.pos].values,
                            y=-np.log10(not_additional_highlight_data[self.pName].values) if self.logp
                            else not_additional_highlight_data[self.pName].values,
                            mode="markers",
                            showlegend=showlegend,
                            name="Chr%i" % chromo,
                            marker={
                            ...
                            text=hover_text_not_additional_highlight
                        )
                    )

                    data_to_plot.append(
                        go.Scattergl(
                            x=additional_highlight_data[self.pos].values,
                            y=-np.log10(additional_highlight_data[self.pName].values) if self.logp
                            else additional_highlight_data[self.pName].values,
                            mode="markers",
                            showlegend=showlegend,
                            name="Chr%i with additional highlight" % chromo,
                            marker={
                                'color': 'orange',
                                'size': point_size,
                                'opacity': 1,
                            ...
                            text=hover_text_additional_highlight
                        )
                    )
